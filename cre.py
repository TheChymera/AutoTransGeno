def cre(datadir, feature_name, extract_from):
	...
	from draw import new_track, draw_digest1, add_to_track
	...
	main_record, main_record_file = extract_feature(sequence_id=extract_from, data_dir="/home/chymera/data/CreBLseq/Cre_aj627603/", feature_names=feature_name, write_file=True)
	...
	hit_track, hit_features = new_track(gdd, construct_name+" hit alignments", smalltick=10)
	add_to_track(hit_features, datadir+"cre_f1.fasta", main_record_file, annotation="cre_f1", feature_color=colors.cornflower)
	add_to_track(hit_features, datadir+"cre_r1.fasta", main_record_file, annotation="cre_r1", feature_color=colors.cornflower)
